    ...
    public function formatear() {
        ...
        foreach ($this->listado as $d) {
            ...
            if ($d['cantidad'] != '') {
                ...
                $dato = new \Base2\Celda($d['dinero']);
                $dato->formatear_dinero();
                $conteo_por_tipo_dato['dinero']++;
            } elseif ($d['porcentaje'] != '') {
                $dato = new \Base2\Celda($d['porcentaje']);
